...
class ExportLetUenTest(unittest.TestCase):
    def test_cli_export_letuen_writes_all_surfaces_and_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            cinema_manifest = json.loads((output_dir / "cinema" / "cinema-data.json").read_text(encoding="utf-8"))
            latest_manifest = json.loads((output_dir / "demo-data.latest.json").read_text(encoding="utf-8"))
            report_html = (output_dir / "report" / "index.html").read_text(encoding="utf-8")
            self.assertEqual(cinema_manifest["run_id"], run_dir.name)
            self.assertEqual(latest_manifest["run_id"], run_dir.name)
            self.assertEqual(cinema_manifest["skill_preview"][0]["id"], "demo-skill")
            self.assertNotIn("composition_plan.json", report_html)
